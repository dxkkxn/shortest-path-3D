...
#     glTranslatef(0., 0., 1.)
#     sph = gluNewQuadric()
#     gluSphere(sph, 1.0, 20,20)
#     glPopMatrix()

def main():
    # f.pack(side = 'top')
    o = Opengl(width = 512, height = 512, double = 1, depth = 1)
    root = o.master
    init()
    f = tk.Frame(root)
    o.master = root
    o.redraw = display
    quit = tk.Button(f, text = 'Quit', command = sys.exit)
    quit.pack({'side':'top', 'side':'left'})
    help = tk.Button(f, text = 'Help', command = o.help)
    help.pack({'side':'top', 'side':'left'})
    reset = tk.Button(f, text = 'Reset', command = o.reset)
    ...
    # o.help()
    root.mainloop()
    # TODO: glutKeyboardFunc(keyboard)
